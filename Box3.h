...
namespace tgx
{
	...
	***************************************************************/
	template<typename T> struct Box3
	{
		...
		* default copy ctor.
		**/
		constexpr Box3(const Box3<T>& B) = default;


		/** 
		* default assignement operator. 
		**/
		constexpr Box3<T>& operator=(const Box3<T>& B) = default;
		...
		**/
		inline void operator|(const Vec3<T>& v) const
			{
			Box3<T> R;
			if (isEmpty())
				{
		...
		**/
		inline void operator+(Vec3<T>  V) const
			{
