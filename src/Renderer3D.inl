...
namespace tgx
{
    ...
    *********************************************************/
        ...
        template<typename color_t, Shader LOADED_SHADERS, typename ZBUFFER_t> TGX_NOINLINE
        void Renderer3D<color_t, LOADED_SHADERS, ZBUFFER_t>::drawMesh(const Mesh3D2<color_t>* mesh, bool use_mesh_material)
            {
            if (!_validDraw()) return;
        ...
        template<typename color_t, Shader LOADED_SHADERS, typename ZBUFFER_t> TGX_NOINLINE
        void Renderer3D<color_t, LOADED_SHADERS, ZBUFFER_t>::drawMesh(const Mesh3D2_16<color_t>* mesh, bool use_mesh_material)
            {
            if (!_validDraw()) return;
            if ((mesh == nullptr) || (mesh->meshlets == nullptr) || (mesh->payload == nullptr) || (mesh->materials == nullptr)) return;
        ...
        template<typename color_t, Shader LOADED_SHADERS, typename ZBUFFER_t>  TGX_NOINLINE
        void Renderer3D<color_t, LOADED_SHADERS, ZBUFFER_t>::_drawMesh(const int RASTER_TYPE, const Mesh3D2<color_t>* mesh, bool use_mesh_material)
            {
            const bool bbox_uninitialized = ((mesh->bounding_box.minX == 0) && (mesh->bounding_box.maxX == 0) &&
                                             (mesh->bounding_box.minY == 0) && (mesh->bounding_box.maxY == 0) &&
                                             (mesh->bounding_box.minZ == 0) && (mesh->bounding_box.maxZ == 0));
            ...
            for (uint16_t mli = 0; mli < mesh->nb_meshlets; mli++)
                ...
                if (current_material_index != meshlet.material_index)
                    ...
                    const MeshMaterial3D2<color_t>& material = mesh->materials[current_material_index];
        ...
        template<typename color_t, Shader LOADED_SHADERS, typename ZBUFFER_t>  TGX_NOINLINE
        void Renderer3D<color_t, LOADED_SHADERS, ZBUFFER_t>::_drawMesh(const int RASTER_TYPE, const Mesh3D2_16<color_t>* mesh, bool use_mesh_material)
            {
            const bool bbox_uninitialized = ((mesh->bounding_box.minX == 0) && (mesh->bounding_box.maxX == 0) &&
                                             (mesh->bounding_box.minY == 0) && (mesh->bounding_box.maxY == 0) &&
                                             (mesh->bounding_box.minZ == 0) && (mesh->bounding_box.maxZ == 0));
            ...
            for (uint16_t mli = 0; mli < mesh->nb_meshlets; mli++)
                ...
                if (current_material_index != meshlet.material_index)
                    {
                    current_material_index = meshlet.material_index;
                    const MeshMaterial3D2_16<color_t>& material = mesh->materials[current_material_index];
                    current_texture = material.texture;
                    _uni.tex = (const Image<color_t>*)current_texture;
