...
- `tgx_image_cli.py`: command-line converter for scripts and batch work.
- `tgx_image_gui.py`: graphical wrapper for interactive use.
...
```bash
python tools/tgx_image_cli.py logo.png --format RGB565 --name logo --output-dir generated
```

You can also give the generated file path directly:

```bash
python tools/tgx_image_cli.py diffuse.png --format RGB565 --name diffuse_texture --output generated/diffuse_texture.h
```
...
```bash
python tools/tgx_image_cli.py panel.png --format RGB32 --layout split --resize 128x64 --resample lanczos --name panel_img --output-dir generated
```

Texture-style export with vertically flipped rows:

```bash
python tools/tgx_image_cli.py diffuse.png --format RGB565 --flip-y --name diffuse_texture --output-dir generated
```
...
Run `python tools/tgx_image_cli.py --help` to see the same options from the command line.

## GUI

```bash
python tools/tgx_image_gui.py
```
