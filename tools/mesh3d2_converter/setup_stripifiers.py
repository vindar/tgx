...
EXTERNAL = TOOL_ROOT / "external_libs"
...
def compile_ga_eax() -> Path:
    ...
    if os.name == "nt":
        ...
        srcs = " ".join(GA_EAX_SOURCES)
        cmd = f'call "{vc}" -arch=x64 >nul && cd /d "{GA_EAX_SRC}" && cl /nologo /O2 /EHsc {srcs} /Fe:"{exe}"'
        run_msvc(cmd)
    else:
...
def compile_lkh() -> Path | None:
    ...
    if not lkh_src:
        print("No LKH 2.x source tree found in tools/mesh3d2_converter/external_libs/LKH. LKH support is optional.")
        return None
