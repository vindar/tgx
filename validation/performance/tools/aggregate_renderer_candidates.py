...
OUT = ROOT / "tmp" / "renderer_layout_investigation"
...
def iter_run_ids(prefix):
    for path in sorted(HV_PARSED.glob("*_benchmark_global.csv")):
        ...
        if board and label.startswith(f"{board}_"):
            label = label[len(board) + 1:]
        if meta.get("parse_mode") == "benchmark" and label.startswith(prefix):
            yield run_id
...
def aggregate(prefix):
    ...
    for run_id in iter_run_ids(prefix):
        meta = metadata(run_id)
        board = meta.get("board", "")
        label = meta.get("label") or run_id
        if board and label.startswith(f"{board}_"):
        ...
        if deltas:
            ...
            summary_rows.append({
                ...
                "regress_lt_minus_5pct": sum(1 for x in values if x <= -5.0),
    ...
    pico2 = [r for r in summary_rows if r.get("board") == "pico2"]
    write_csv(OUT / "pico2_candidate_summary.csv", pico2)
    write_csv(OUT / "pico2_subtest_matrix.csv", [r for r in subtest_rows if r.get("board") == "pico2"])

    cross = [r for r in summary_rows if r.get("board") != "pico2"]
    write_csv(OUT / "cross_board_candidate_summary.csv", cross)
    write_csv(OUT / "cross_board_subtest_matrix.csv", [r for r in subtest_rows if r.get("board") != "pico2"])

    print(f"wrote renderer candidate aggregates for prefix {prefix!r}")


if __name__ == "__main__":
    aggregate(sys.argv[1] if len(sys.argv) > 1 else "renderer_")
