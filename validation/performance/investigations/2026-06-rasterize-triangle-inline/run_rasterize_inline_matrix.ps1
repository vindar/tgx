param(
    ...
    [string[]]$Variants = @("default", "raster_empty", "raster_noinline", "raster_noinline_noclone"),
    [int]$Rounds = 1,
    [switch]$Benchmark,
    [switch]$Examples,
    [switch]$CompileOnly,
    [int]$BenchmarkTimeoutSeconds = 210,
    [int]$ExampleTimeoutSeconds = 28,
    [int]$RetryCount = 1
)
...
$outRoot = Join-Path $root "validation\performance\investigations\2026-06-rasterize-triangle-inline"
...
$exampleCfg = @{
    "teensy41" = @(
        @{ Name = "mars"; Sketch = "examples\Teensy4\3D\mars"; Baud = 9600 },
        @{ Name = "test-shading"; Sketch = "examples\Teensy4\3D\test-shading"; Baud = 9600 },
        @{ Name = "test-texture"; Sketch = "examples\Teensy4\3D\test-texture"; Baud = 9600 },
        @{ Name = "buddha"; Sketch = "examples\Teensy4\3D\buddha"; Baud = 9600 },
    ...
    "core2" = @(
        ...
        @{ Name = "donkeykong"; Sketch = "examples\M5Stack\donkeykong"; Baud = 115200 },
        @{ Name = "scream"; Sketch = "examples\M5Stack\scream"; Baud = 115200 },
        ...
        @{ Name = "spotlights_room"; Sketch = "examples\M5Stack\spotlights_room"; Baud = 115200 }
    )
    "cores3" = @(
        @{ Name = "borg_cube"; Sketch = "examples\M5Stack\borg_cube"; Baud = 115200 },
        @{ Name = "donkeykong"; Sketch = "examples\M5Stack\donkeykong"; Baud = 115200 },
    ...
    "pico2" = @(
        ...
        @{ Name = "bunny_fig"; Sketch = "examples\Pico_RP2040_RP2350\bunny_fig"; Baud = 115200 },
        @{ Name = "scream"; Sketch = "examples\Pico_RP2040_RP2350\scream"; Baud = 115200 },
        ...
        @{ Name = "spotlights_room"; Sketch = "examples\Pico_RP2040_RP2350\spotlights_room"; Baud = 115200 }
    )
}
...
function Invoke-Capture {
    ...
    Write-Host "[rasterinline] $Board $Label flags='$ExtraFlags'"
    & powershell @args
    return $LASTEXITCODE
}
...
foreach ($round in 1..$Rounds) {
    foreach ($board in $Boards) {
        ...
        foreach ($variant in $Variants) {
            ...
            if ($Benchmark) {
                $label = "rasterinline_r${round}_${variant}_benchmark_${board}"
                ...
                $rows.Add([pscustomobject]@{ round=$round; board=$board; variant=$variant; kind="benchmark"; example="benchmark"; label=$label; flags=$flags; exit_code=$exitCode }) | Out-Null
            }
            if ($Examples) {
                foreach ($ex in $exampleCfg[$board]) {
                    $label = "rasterinline_r${round}_${variant}_example_$($ex.Name)_${board}"
                    ...
                    $rows.Add([pscustomobject]@{ round=$round; board=$board; variant=$variant; kind="example"; example=$ex.Name; label=$label; flags=$flags; exit_code=$exitCode }) | Out-Null
                }
            }
        }
    }
...
$path = Join-Path $summaryDir "rasterize_inline_matrix_${suffix}_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv"
$rows | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $path
Write-Host "[rasterinline] summary=$path"
