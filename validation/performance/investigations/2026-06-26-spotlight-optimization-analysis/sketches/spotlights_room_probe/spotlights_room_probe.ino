...
#ifndef TGX_LIGHT_PROBE_MODE
#define TGX_LIGHT_PROBE_MODE 0
#endif

// Probe modes:
// 0 = two moving point lights with local specular
// 1 = two moving point lights without local specular
// 2 = one moving point light with local specular
// 3 = two moving cone spotlights with local specular
...
#if TGX_LIGHT_PROBE_MODE == 3
    renderer.setSpotLightDirection(0, warm.target - warm.position);
    renderer.setSpotLightDirection(1, cool.target - cool.position);
#endif
    }
...
#if TGX_LIGHT_PROBE_MODE == 1
    renderer.setSpotLightCount(2);
    renderer.setSpotLight(0, warm.position, 5.8f, WARM_DIFFUSE, RGBf(0.0f, 0.0f, 0.0f));
    renderer.setSpotLight(1, cool.position, 5.6f, COOL_DIFFUSE, RGBf(0.0f, 0.0f, 0.0f));
#elif TGX_LIGHT_PROBE_MODE == 2
...
#elif TGX_LIGHT_PROBE_MODE == 3
    renderer.setSpotLightCount(2);
    renderer.setSpotLight(0, warm.position, warm.target - warm.position,
                          6.0f, 34.0f, 22.0f, WARM_DIFFUSE, WARM_SPECULAR);
    renderer.setSpotLight(1, cool.position, cool.target - cool.position,
                          5.8f, 38.0f, 24.0f, COOL_DIFFUSE, COOL_SPECULAR);
    renderer.setSpotLightCone(0, 34.0f, 22.0f);
    renderer.setSpotLightCone(1, 38.0f, 24.0f);
#else
    renderer.setSpotLightCount(2);
    renderer.setSpotLight(0, warm.position, 5.8f, WARM_DIFFUSE, WARM_SPECULAR);
    renderer.setSpotLight(1, cool.position, 5.6f, COOL_DIFFUSE, COOL_SPECULAR);
...
void updateFPS(uint32_t render_us)
    ...
    if (now - fps_last_ms >= 1000)
        {
        Serial.print("spotlights_room_probe mode=");
        Serial.print(TGX_LIGHT_PROBE_MODE);
        Serial.print(" Pico fps=");
