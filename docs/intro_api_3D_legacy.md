@page intro_api_3D_legacy Previous 3D API tutorial

@warning This is the previous version of the 3D tutorial, kept temporarily for comparison while the main tutorial is reorganized.
...
All solid primitive calls (`drawTriangle()`, `drawCube()`, `drawSphere()`, `drawCylinder()`, `drawMesh()`...) take
coordinates in model space. In a render loop, update the model matrix just before drawing the object it belongs to.
...
## Mesh3Dv2, Mesh3D and generated models
...
| \ref tgx::Renderer3D::drawCylinder "drawCylinder()" | Draw a generated unit cylinder, optionally textured on the side and caps. |
| \ref tgx::Renderer3D::drawCone "drawCone()" | Draw a generated unit cone, optionally textured on the side and bottom cap. |
| \ref tgx::Renderer3D::drawTruncatedCone "drawTruncatedCone()" | Draw a generated cone frustum with separate bottom and top radii. |
...
`drawCylinder()`, `drawCone()` and `drawTruncatedCone()` use `nb_sectors` to control the circular tessellation. They
are unit primitives centered on the origin in model space, so the model matrix is normally used to scale, rotate and
place them in the scene. Caps are enabled by default and can be disabled with the `bottom_cap` and `top_cap`
parameters when an open shape is needed.
...
// Spot light: position, direction, range, outer cone angle, diffuse color.
renderer.setSpotLight(1, tgx::fVec3(-1.0f, 1.5f, -0.5f),
                      tgx::fVec3(0.4f, -0.8f, -0.2f),
                      4.0f, 28.0f, tgx::RGBf(0.25f, 0.45f, 1.0f));
~~~

The `range` parameter controls both the maximum influence distance and the smooth attenuation. A value less than or
equal to zero gives an infinite-range light, but finite ranges are usually more useful and faster visually to tune.
Spot lights use a cone half-angle in degrees; values greater than or equal to 180 behave like point lights. An
optional `specularColor` parameter enables local specular highlights for that light; the default black value disables
the local specular contribution.
