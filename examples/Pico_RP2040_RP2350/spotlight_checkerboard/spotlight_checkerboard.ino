...
#define CHECKERBOARD_NX 10
#define CHECKERBOARD_NZ 8
#endif
...
void updateFPS(uint32_t render_us)
    ...
    if (now - fps_last_ms >= 1000)
        {
        Serial.print("spotlight_checkerboard Pico fps=");
...
void setupDMA()
    ...
    if (use_dma)
        {
        tft.startWrite();
        Serial.println("spotlight_checkerboard Pico display DMA enabled");
        }
    else
        {
        Serial.println("spotlight_checkerboard Pico display DMA unavailable, using pushImage");
        }
    }
