...
void updateFPS(uint32_t render_us)
    ...
    if (now - fps_last_ms >= 1000)
        {
        Serial.print("pointlight_textured_meshes M5Stack fps=");
...
void setup()
    ...
    if (!allocateRenderBuffers((size_t)render_lx * (size_t)render_ly))
        {
        Serial.println("pointlight_textured_meshes M5Stack buffer allocation failed");
    ...
    if (use_dma)
        ...
        lcd.initDMA();
        lcd.startWrite();
        Serial.println("pointlight_textured_meshes M5Stack display DMA enabled");
        }
    else
        {
        Serial.println("pointlight_textured_meshes M5Stack display DMA unavailable, using pushImage");
        }
